...
bool visit_type_q_obj_read_mem_arg_members(Visitor *v, q_obj_read_mem_arg *obj, Error **errp)
{
    ...
    if (visit_optional(v, "qom", &has_qom)) {
...
{
    if (!visit_type_int(v, "time-ns", &obj->time_ns, errp)) {
        return false;
    }
    if (!visit_type_int(v, "event-id", &obj->event_id, errp)) {
        return false;
    }
    return true;
}
...
{
    bool has_gpio = !!obj->gpio;

    if (!visit_type_str(v, "device-name", &obj->device_name, errp)) {
        return false;
    }
    if (visit_optional(v, "gpio", &has_gpio)) {
